...
class _EntryDetailScreenState extends State<EntryDetailScreen> {
  ...
  late Entry entry = getDefaultEntry();
  ...
  //
  void fetchEntry(dynamic entryId) {
    setState(() => isLoading = true);
    apiController.fetchData('entry/detail/$entryId').then((data) {
      setState(() {
        isLoading = false;
        entry =  Entry.fromJson(data["data"]);
      });
    }).catchError((error) {
        ...
        body: Skeletonizer(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Liste des vêtements',
                ...
                Flexible(
                  child: entry.clothes != null && entry.clothes!.isNotEmpty
                      ? ListView(
                          shrinkWrap:
                              true, // Réduit l'espace occupé par ListView
                          children: entry.clothes!
                              .map((item) => buildItem(
                      ...
                      : const EmptyList(
                          wording:
                              'Aucun vêtement disponible'), // Message si la liste est vide
                ),
                const SizedBox(
                    height: 16.0), // Réduit l'espace entre la liste et l'offre
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
                'Une remise de $discounPercentage % a été appliqué sur le montant total!',
